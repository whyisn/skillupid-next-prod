...
export default function LearnClient({ enrollmentId, courseId, modules, activeModuleId }) {
  ...
  // tandai progress selesai saat video selesai
  const handleCompleted = async () => {
    if (!active) return;
  ...
  // cek ke backend apakah kuis modul aktif sudah lulus (>= 4 benar)
  const checkQuizPassed = async (quizId) => {
  ...
  const nextModule = async () => {
    ...
    const idx = modules.findIndex((m) => m.id === active.id);
    const next = modules[idx + 1];
    if (!next) return; // sudah modul terakhir
    ...
    try {
      ...
      // kalau sudah lulus kuis → boleh lanjut modul berikutnya
